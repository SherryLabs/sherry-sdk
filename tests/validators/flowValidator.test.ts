...
describe('FlowValidator', () => {
        ...
        actions: [
            {
                ...
                type: 'http',
                label: 'Start',
                path: 'https://api.example.com/start',
                params: [],
                nextActions: [{ actionId: 'decision' }],
            } as NestedHttpAction,
            {
                id: 'decision',
                type: 'decision',
    ...
    describe('validateFlow', () => {
        ...
        it('should throw error for a flow without initialActionId', () => {
            ...
            expect(() => FlowValidator.validateFlow(invalidFlow as ActionFlow)).toThrow(
        ...
        it('should throw error for a flow with non-existent initialActionId', () => {
            ...
            expect(() => FlowValidator.validateFlow(invalidFlow)).toThrow(
                /Initial action.*not found/,
            );
        });
                ...
                actions: [
                    {
                        ...
                        label: 'Start',
                        path: 'https://api.example.com/start',
                        nextActions: [{ actionId: 'middle' }],
                    } as NestedHttpAction,
                    {
                        id: 'middle',
                        type: 'http',
                        label: 'Middle',
                        path: 'https://api.example.com/middle',
                        nextActions: [{ actionId: 'end' }],
                    } as NestedHttpAction,
                    {
                        id: 'end',
                        type: 'completion',
    ...
    describe('validateAction', () => {
        ...
        it('should throw error for a decision without options', () => {
            ...
            expect(() => (FlowValidator as any).validateAction(invalidDecision, validFlow)).toThrow(
                InvalidMetadataError,
            );
            expect(() => (FlowValidator as any).validateAction(invalidDecision, validFlow)).toThrow(
                /must have options/,
