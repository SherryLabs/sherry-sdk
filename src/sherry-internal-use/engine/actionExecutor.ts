...
export async function executeAction(action: BlockchainAction | TransferAction | HttpAction): Promise<any> {
  try {
...
async function executeBlockchainAction(action: BlockchainAction): Promise<any> {
  console.log(`Executing blockchain action: ${action.functionName} on contract ${action.address}`);
