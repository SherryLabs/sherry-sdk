...
export class TransferActionValidator {
    ...
    private static validateChains(chains: ChainContext): void {
        ...
        if (chains.destination !== undefined) {
            if (typeof chains.destination !== 'string' || !validChains.includes(chains.destination)) {
    ...
    private static validateRecipient(action: TransferAction): void {
        ...
        if (action.recipient !== undefined) {
            ...
            if (!['select', 'input'].includes(action.recipient.inputType || '')) {
                throw new InvalidMetadataError(`Invalid recipient input type: ${action.recipient.inputType}`);
            }
            
            if (action.recipient.inputType === 'select' && (!Array.isArray(action.recipient.options) || action.recipient.options.length === 0)) {
                throw new InvalidMetadataError('Recipient select options must be a non-empty array');
            }
        }
    ...
    private static validateAmount(action: TransferAction): void {
        ...
        if (action.amountConfig !== undefined) {
            ...
            if (action.amountConfig.inputType && !['select', 'radio', 'input'].includes(action.amountConfig.inputType)) {
                throw new InvalidMetadataError(`Invalid amount input type: ${action.amountConfig.inputType}`);
            }
            
            if (['select', 'radio'].includes(action.amountConfig.inputType || '') && 
                (!Array.isArray(action.amountConfig.options) || action.amountConfig.options.length === 0)) {
                throw new InvalidMetadataError('Amount options must be a non-empty array');
            }
            
            if (action.amountConfig.defaultValue !== undefined && 
