import { BlockchainActionMetadata, BlockchainAction, UIParameter, TransferAction } from "../interface/blockchainAction";
  ...
  static validateUIParameter(param: UIParameter, value: any, name: string): void {
    if (param.minValue !== undefined && value < param.minValue) {
      throw new InvalidMetadataError(
        `${name}: Value must be greater than ${param.minValue}`
      );
    }

    if (param.maxValue !== undefined && value > param.maxValue) {
      throw new InvalidMetadataError(
        `${name}: Value must be less than ${param.maxValue}`
      );
    }
