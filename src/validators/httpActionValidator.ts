import { HttpAction, HttpParameter, SelectParameter, RadioParameter, StandardParameter } from "../interface/httpAction";
...
export class HttpActionValidator {
  static validateHttpAction(action: HttpAction): HttpAction {
    ...
    //this.validateMethod(action.method);
  ...
  private static validateParameters(parameters?: HttpParameter[]): HttpParameter[] {
    ...
    return parameters.map(param => {
      ...
      switch (param.type) {
        ...
        default:
          throw new InvalidMetadataError(`Invalid parameter type: ${(param as any).type}`);
      }
    });
  }
    ...
    if (param.defaultValue !== undefined) {
      if (!param.options.some(opt => opt.value === param.defaultValue)) {
        throw new InvalidMetadataError(
          `Invalid default value for parameter ${param.name}`
        );
      }
    }
  ...
  private static validateStandardParameter(param: StandardParameter): StandardParameter {
    ...
    if (param.pattern) {
      try {
        ...
        throw new InvalidMetadataError(
          `Invalid regex pattern for parameter ${param.name}`
        );
      }
    }
  ...
  static isHttpAction(action: any): action is HttpAction {
    ...
    return action.validatedParams.every((param: HttpParameter) => 
      param &&
