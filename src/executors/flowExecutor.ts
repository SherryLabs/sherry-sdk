...
export class FlowExecutor {
     ...
     * @returns Resultado de la ejecución
     */
    private async executeBlockchainAction(action: NestedAction, actionData?: any): Promise<ActionResult> {
        ...
        const processedParams = action.params ? 
            ...
            (actionData?.params ? actionData.params : []);
        
        // En una implementación real, aquí realizarías la llamada a la blockchain
        // Por ahora, simular una respuesta exitosa
        console.log(`[Blockchain] Executing ${action.functionName} on ${action.address} with params:`, processedParams);
     ...
     * @returns Resultado de la ejecución
     */
    private async executeTransferAction(action: NestedAction, actionData?: any): Promise<ActionResult> {
        ...
        console.log(`[Transfer] Sending ${action.amount} to ${to} on chain ${action.chains.source}`);
