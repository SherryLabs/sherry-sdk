...
// Extend AbiParameter with our custom properties - this avoids type conflicts
interface ExtendedAbiParameter extends AbiParameter {
    originalName?: string;
    value?: any;
}
 ...
 */
function validateParametersMatchAbi(abiParams: ExtendedAbiParameter[], configParams: BlockchainParameter[]): void {
    try {
 ...
 */
function validateParameterTypeCompatibility(configParam: BlockchainParameter, abiParam: ExtendedAbiParameter): void {
    try {
        ...
        const isConfigArray = configParam.type === 'array';
        ...
        if (error instanceof SherryValidationError) {
            ...
            throw new SherryValidationError(`Error validating parameter type compatibility: ${error.message}`);
        }
    }
}
...
function applyParamValues(abiParams: ExtendedAbiParameter[], paramsValue?: any[]): void {
    try {
        ...
        if (error instanceof SherryValidationError) {
            ...
            throw new SherryValidationError(`Error applying parameter values: ${error.message}`);
        }
    }
}
...
function applyParamLabels(abiParams: ExtendedAbiParameter[], paramsLabel?: string[]): void {
    try {
        ...
        if (error instanceof SherryValidationError) {
            ...
            throw new SherryValidationError(`Error applying parameter labels: ${error.message}`);
        }
    }
}
...
export function validateBlockchainAction(action: BlockchainActionMetadataV2): void {
    try {
        ...
        if (error instanceof SherryValidationError) {
            ...
            throw new SherryValidationError(`Error validating blockchain action: ${error.message}`);
        }
    }
}
...
export function processBlockchainAction(action: BlockchainActionMetadataV2): BlockchainActionV2 {
    try {
        ...
        throw new SherryValidationError(
            `Error processing blockchain action '${action.label || action.functionName}': ${error.message}`
        );
    }
}
