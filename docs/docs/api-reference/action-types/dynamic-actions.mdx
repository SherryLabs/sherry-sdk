...
export async function POST(req: NextRequest) {
  try {
    ...
    const protocolYields = await fetchProtocolYields([
      'AAVE', 'Compound', 'Yearn', 'Convex', 'Curve', 'TraderJoe'
    ]);
...
function calculateOptimalAllocation(protocols: any[], amount: number, duration: number, autoCompound: boolean) {
  ...
  params: [
    {
      ...
      label: 'From Token',
      type: 'select',
      options: [/* tokens on Avalanche */]
    },
    {
      name: 'toToken', 
      label: 'To Token',
      type: 'select',
      options: [/* tokens on Celo */]
    },
    {
      name: 'amount',
      label: 'Amount',
