    ...
    protocols.map(async protocol => {
      try {
        const response = await fetch(
          `https://api.defillama.com/yields/pool/${protocol}`,
          { timeout: 5000 }
        );
...
function filterByRiskProfile(protocols: any[], riskTolerance: string) {
  ...
  return protocols.filter(
    p => p.riskScore <= threshold.maxRisk && p.apy >= threshold.minAPY
  );
}
...
export async function POST(req: NextRequest) {
  ...
  if (userAddress) {
    ...
    if (userRequestCount > 100) { // Max 100 requests per hour per address
      return NextResponse.json(
