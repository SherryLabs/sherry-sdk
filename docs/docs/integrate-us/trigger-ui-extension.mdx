...
> **⚠️ Beta Notice**: TriggerKit is currently in active development. Some APIs and configurations may change in future releases as we refine the extension integration experience.
...
async function handleWalletCommunication(
  ...
  console.log(`[Sherry EVM] 🧠 Handling ${type} with wallet: ${wallet}`);
...
export async function initSlinksObserver() {
  ...
  console.log('[trigger-kit] 🧠 Starting setupSlinksObserverEVM...');
