...
> **⚠️ Beta Notice**: TriggerKit is currently in active development. Some properties and APIs may change in future releases. The bypass proxy feature is temporary while we fine-tune our infrastructure.
  ...
  return (
    <div style={{ maxWidth: '450px', margin: '20px auto' }}>
      <MiniAppComponent
        ...
        adapter={adapter}
        bypassProxy={true} // ⚠️ Beta only - while we fine-tune proxy infrastructure
      />
    </div>
  );
}
...
> **🔄 Development Notice**: Some props may change in future releases as we refine the API.
...
- `bypassProxy`: Boolean - Direct fetch (⚠️ Beta only, beware of CORS)
...
2. **CORS Issues**
   ...
   <MiniAppComponent
     url="https://app.sherry.social/action/swap"
     adapter={adapter}
     bypassProxy={true} // ⚠️ Beta only - may cause CORS issues
   />
   ```
